...
if not raw.strip():
    print(f"error: no taste learnings found (looked at {TASTE}). Run /taste or cmd learn-taste first.", file=sys.stderr); sys.exit(2)
...
def T(x, y, s, size=15, weight="normal", anchor="start", fill="var(--fg)", rot=None):
...
def title_block(w, t, sub=None, reserve=0):
    ...
    if sub:
        ...
        st = T(22, 46, sub_, 16, fill="var(--muted)")
...
def hbars(title, items, xlabel, sub=None, w=900, color="var(--bar)", ylabel=None, lw=250, sort=True):
    ...
    for k, (lab, v, hov) in enumerate(items):
        ...
        out.append(T(xs(v) + 8, y + rh / 2 + 5, fmt(v) if isinstance(v, int) or float(v).is_integer() else f"{v:.2f}".rstrip("0").rstrip("."), 12, "600", fill="var(--fg)"))
    ...
    if ylabel: out.append(T(16, top + rh * len(items) / 2, ylabel, 13, "bold", "middle", rot=True))
    return "".join(out) + "</svg>"
...
def stacked_h(title, rowsx, series, colmap, xlabel, ylabel, sub=None, w=900, unit="", total=True):
    ...
    for v in range(0, int(mx) + 1, max(1, math.ceil(mx / 9))):
        out.append(L(xs(v), top - 8, xs(v), top + rh * len(rowsx), "var(--grid)", 1)); out.append(T(xs(v), top + rh * len(rowsx) + 18, v, 16, anchor="middle"))
    for k, (lab, d) in enumerate(rowsx):
        ...
        for sname in series:
            ...
            x += v
        if total: out.append(T(xs(x) + 8, y + rh / 2 + 5, x, 12, "600", fill="var(--fg)"))
    ...
    out.append(T((L0 + w - 40) / 2, h - 12, xlabel, 13, "bold", "middle")); out.append(T(16, top + rh * len(rowsx) / 2, ylabel, 13, "bold", "middle", rot=True))
    return "".join(out) + "</svg>"
...
def stacked_v(title, cats, series, colmap, xlabel, ylabel, sub=None, w=900, h=360, reserve=0):
    ...
    for v in ticks:
        out.append(L(L0, ys(v), w - 30, ys(v), "var(--grid)", 1)); out.append(T(L0 - 8, ys(v) + 4, (f"{v:.2f}".rstrip("0").rstrip(".") if mx < 6 else fmt(v)), 16, anchor="end"))
    for k, (lab, d) in enumerate(cats):
        ...
        if len(cats) > 8: out.append(f"<text x='{x + cw * 0.35:.0f}' y='{h - B + 8:.0f}' text-anchor='end' fill='var(--fg)' style='font-size:14px' transform='rotate(-45 {x + cw * 0.35:.0f} {h - B + 8:.0f})'>{esc(lab)}</text>")
        else: out.append(T(x + cw * 0.35, h - B + 16, lab, 15, anchor="middle"))
...
[data-theme=dark]{--bg:#0b0e13;--surface:#12161d;--surface2:#1a2029;--border:rgba(255,255,255,.10);--fg:#f2f4f8;--muted:#aab2c2;--muted2:#8e97a8;--grid:rgba(255,255,255,.10);--axis:rgba(255,255,255,.5);--bar:#d5dce6;--bar2:#5a6475;--onbar:#0b0e13;--accent:#6ea6ff;--accent-fg:#0b0e13;--shadow:0 1px 0 rgba(255,255,255,.03) inset,0 8px 24px rgba(0,0,0,.35)}
[data-theme=light]{--bg:#f6f7f9;--surface:#ffffff;--surface2:#f1f3f6;--border:rgba(15,23,42,.12);--fg:#0f172a;--muted:#4b5563;--muted2:#6b7280;--grid:rgba(15,23,42,.07);--axis:rgba(15,23,42,.45);--bar:#1f2937;--bar2:#b9c0cc;--onbar:#ffffff;--accent:#2563eb;--accent-fg:#fff;--shadow:0 1px 2px rgba(15,23,42,.06)}
...
.flex{display:flex;gap:16px;align-items:flex-start}.legend{flex:0 0 220px;background:var(--surface);background-image:var(--gloss);border-radius:16px;padding:12px 14px;font-size:13px;margin-top:12px;box-shadow:var(--edge),var(--lift)}.legend b{color:var(--muted);font-weight:600;font-size:11px;letter-spacing:.06em;text-transform:uppercase}.legend div{margin:8px 0}.leg-item{cursor:pointer}.leg-item.solo .dot{outline:2px solid var(--accent);outline-offset:1px}.leg-item.inverted .dot{opacity:.3}.leg-item:hover .dot{box-shadow:0 0 0 1px rgba(0,0,0,.15) inset,0 0 0 2px var(--accent)}
...
table{border-collapse:separate;border-spacing:0;width:100%;margin:8px 0 16px;background:var(--surface);background-image:var(--gloss);border-radius:14px;overflow:hidden;font-size:13px;box-shadow:var(--edge),var(--lift)}th{background:var(--surface2);color:var(--fg);opacity:.85;font-weight:600;font-size:11px;letter-spacing:.05em;text-transform:uppercase;text-align:center;padding:9px 10px;border-bottom:1px solid var(--border)}td{padding:7px 10px;border-bottom:1px solid var(--border);vertical-align:top;text-align:center;transition-property:background-color;transition-duration:.12s}tr:last-child td{border-bottom:0}tr:hover td{background:var(--surface2)}.num{text-align:center}
...
.tt{position:fixed;z-index:50;pointer-events:none;opacity:0;background:var(--surface2);color:var(--fg);border-radius:10px;padding:8px 10px;font-size:12px;line-height:1.5;box-shadow:var(--edge),var(--lift);min-width:160px;max-width:360px;white-space:normal;overflow-wrap:break-word;max-height:420px;overflow-y:auto;transition-property:opacity;transition-duration:.12s}.tt .dot{margin-right:6px}.tt div{display:flex;justify-content:space-between;gap:14px}.tt b{margin-left:auto}.tt .tt-h{display:block;font-weight:600;margin-bottom:4px;color:var(--fg)}.tt .tt-t{display:block;border-top:1px solid var(--border);margin-top:4px;padding-top:4px;color:var(--muted)}
...
svg.attn g.bar:not(.hot) rect{opacity:.45}svg.attn g.bar:not(.hot) text{opacity:.6}g.bar.hot rect.hb{transform:scaleY(1.18) scaleX(1.01);filter:brightness(1.18) saturate(1.15)}g.bar.hot rect.vb{transform:scaleY(1.04) scaleX(1.08);filter:brightness(1.18) saturate(1.15)}g.bar.hot text{font-weight:700;opacity:1}g.bar text{transition-property:opacity;transition-duration:.2s}
...
def build(SUF, rows, sessions, acts, rows_r=None, gran="day", cut="0000"):
    ...
    habits_chart = flex(stacked_h("Work Habits by Work Area", grid, dl, cols, "Number of learnings", "Work habit", "Learnings per habit, colored by work area · selected range", w=620), legend(cols, "Work area", dc_r))
    habit_defs = "<p class=muted>A learning can show more than one habit." + (f" Too small to chart: {', '.join(f'{d} ({k})' for d, k in dc_all.items() if d not in KEEP)}." if len(dc_all) > len(dc) else "") + "</p><ul>" + "".join(f"<li><b>{t}</b>: {d}</li>" for t, d, _ in TRAITS) + "</ul>"
    ...
    if not rows_r: H.append("<p class=muted style='padding:12px 0'>No learnings were learned in this range. Widen the range to see the file's content; the whole file is still injected into every prompt.</p>")
    else: H.append(two(habits_chart, habit_defs))
    OV["habits"] = flex(stacked_h("Work Habits by Work Area", [(t, {d: sum(1 for r in rows_main if r["domain"] == d and t in r["traits"]) for d in dl}) for t in tl], dl, cols, "Number of learnings", "Work habit", "Learnings per habit, colored by work area · whole file", w=620), legend(cols, "Work area", dc))
    h3("Where the learnings came from")
    ...
    H.append("<p class=charttip>Each learning is matched to the session it was most likely learned from by shared distinctive words. <b>Unmatched</b> = no transcript on disk shared enough distinctive words: the source session was deleted or compacted, or the learner paraphrased the learning beyond recognition.</p>")
    if rows_r: H.append(two(hbars("Learnings by Source", [(k, v, "") for k, v in srcs.items()], "Learnings", ylabel="Source", w=620, lw=230),
                 tseries("Learnings Gained per Week by Source", [(wk[5:], dict(c)) for wk, c in weeks_b.items()], sl, scols, "Week starting", "Learnings", w=620, h=380, leg=legend(scols, "Source", srcs))))
    h3("All learnings")
    ...
    H.append("<script>(function(){const D=" + json.dumps(bdata) + ";const S='" + SUF + "';const f=document.getElementById('bf'+S),t=document.getElementById('bt'+S),p=document.getElementById('bp'+S);"
             ...
             "f.querySelector('[data-k=count]').textContent=R.length+' learnings';p.innerHTML=`<button ${st.page===0?'disabled':''} data-d='-1'>‹ Prev</button><span>Page ${st.page+1} of ${n}</span><button ${st.page>=n-1?'disabled':''} data-d='1'>Next ›</button>`;p.querySelectorAll('button').forEach(b=>b.onclick=()=>{st.page+=+b.dataset.d;render();});}"
    ...
    # ================= INFLUENCE =================
    tab("Influence")
    H.append(kpi([("activations", len(acts), "Moments where the model's reasoning mentioned taste and could be matched to one specific learning."), ("steering", n_steer, "Activations where the sentence went on to change the plan: so, should, instead, before, never."), ("steering share", f"{100*n_steer/max(1,len(acts)):.0f}%", "Of all activations, the share that changed what the model did next."), ("turns that consulted taste", f"{100*sum(s['turns_cite'] for s in sessions)/max(1,sum(s['asst'] for s in sessions)):.1f}%", "Share of all assistant turns whose reasoning mentioned taste at all."), ("learnings consulted", len(rows) - len(never), f"Distinct learnings referenced at least once, out of {len(rows)}."), ("skill invocations", sum(1 for e in skill_events if e["sid"] in {s["sid"] for s in sessions}), "Explicit activate_skill calls, for comparison with implicit taste use.")]))
    ...
    c1 = flex(stacked_h("Taste Activations by Work Habit", hb, ["steering", "mention"], {"steering": "var(--bar)", "mention": "var(--bar2)"}, "Activations", "Work habit", "Steering (dark) vs mention only (light)", w=620), legend({"steering": "var(--bar)", "mention": "var(--bar2)"}, "Kind"))
    tot_t = sum(tc.values()) or 1; tot_a = sum(act_by_trait.values()) or 1
    c2 = (hbars("Activations per Learning by Work Habit", [(t, round(act_by_trait.get(t, 0) / max(1, tc.get(t, 0)), 1), f"{act_by_trait.get(t,0)} activations across {tc.get(t,0)} learnings") for t in tl], "Activations per learning", "Mean consultations per learning · selected range", ylabel="Work habit", w=620, lw=190))
    H.append(two(c1, c2)); OV["infl"] = c1; OV["work"] = c2
    top_b = sorted(rows, key=lambda r: -r["acts"])[:15]
    c3 = hbars("Most Activated Learnings", [(f"#{r['i']} " + r["text"][:28] + "…", r["acts"], r["text"]) for r in top_b], "Activations", "Consultations per learning · selected range", ylabel="Learning", lw=260, w=620)
    ...
    if True:
        ...
        c4 = tseries(f"Taste Activations {PER.title()}", [(l, dict(c)) for l, c in hb_.items()], ["steering", "mention"], {"steering": "var(--bar)", "mention": "var(--bar2)"}, BUCKET_WORD.capitalize(), "Activations", w=620, h=520)
    ...
    for dd, sid, b, q in steer_quotes[:25]: H.append(f"<blockquote><b>{dd} · {sid} · learning #{b+1}</b><br>{esc(q)}</blockquote>")
    H.append("</details>")
    ...
    H.append("<p class=charttip>Two directions, both within the selected range. <b>Creates</b>: new taste learnings gained from this model's sessions. <b>Uses</b>: times this model's reasoning consulted a learning. Both per 100 turns so long and short sessions compare fairly.</p>")
    H.append(table(["Model", "#Turns", "#New learnings created", "#Created per 100 turns", "#Times taste used", "#Used per 100 turns", "#Uses that changed the plan %", "#Taste traffic per 100 turns", "#Share of traffic that is use %"], tw_rows))
    H.append("<p class=charttip><b>Taste traffic</b> = created + used, per 100 turns: how much this model interacts with taste at all. <b>Share that is use</b>: 0% = the model only creates learnings, 100% = it only uses them, 50% = balanced.</p>")
    ...
    def sankey(src_in, cons, w=1240, h=500):
        ...
        out = [svg_open(w, h), title_block(w, "Taste Flow: Sources → taste.md → Consuming Models", "Band width = share within each side · selected range · per-turn rates in the chart below")]
        tot_in, tot_out = sum(src_in.values()), sum(cons.values())
        out.append(T(Lx, top - 30, "IN · learnings created", 13, "bold")); out.append(T(Lx, top - 14, f"{tot_in} learnings gained in this range, by the session that taught them", 11, fill="var(--muted)"))
        out.append(T(Rx, top - 30, "OUT · learnings consulted", 13, "bold", "end")); out.append(T(Rx, top - 14, f"{tot_out} consultations in this range, by the model that used them", 11, fill="var(--muted)", anchor="end"))
        out.append(T(18, top + H_ / 2, "Source of learnings", 12, "bold", "middle", rot=True)); out.append(T(w - 14, top + H_ / 2, "Model using learnings", 12, "bold", "middle", rot=True))
        ...
        out.append(R(Mx - nw / 2, mid_y, nw, mid_h, "var(--fg)", f"taste.md · {sum(src_in.values())} learnings in, {sum(cons.values())} consultations out", 0.9))
        out.append(T(Mx, mid_y - 10, "taste.md", 12, "bold", "middle"))
        yl = mid_y
        for k, (x, y, hh, v) in Ln.items():
            hh2 = mid_h * v / max(1, sum(src_in.values()))
            out.append(R(x, y, nw, hh, scol[k], f"{k}: {100*v/max(1,tot_in):.0f}% of learnings ({v})", 0.95))
            out.append(f"<path d='M{x+nw:.0f},{y:.0f} C{(x+nw+Mx)/2:.0f},{y:.0f} {(x+nw+Mx)/2:.0f},{yl:.0f} {Mx-nw/2:.0f},{yl:.0f} L{Mx-nw/2:.0f},{yl+hh2:.0f} C{(x+nw+Mx)/2:.0f},{yl+hh2:.0f} {(x+nw+Mx)/2:.0f},{y+hh:.0f} {x+nw:.0f},{y+hh:.0f} Z' fill='{scol[k]}' opacity='0.35'><title>{esc(k)} → taste.md: {v} learnings</title></path>")
    ...
    if self_pairs:
        ...
        OV["self"] = two(flex(stacked_h("Consultations: Own Learnings vs Pool by Model", self_pairs, list(sc_cols), sc_cols, "Share of the model's consultations", "Model", "Own learnings vs learnings from other models, Claude Code or Cursor", w=620, unit="%", total=False), legend(sc_cols, "Consulted")),
                     hbars("Self-Preference Ratio by Model", [(m, r[6], f"{r[5]}% of its consultations hit its own learnings; its learnings are {r[3]}% of the file") for m, r in zip([r[0] for r in self_rows], self_rows) if isinstance(r[6], float)], "Own-learning share ÷ own share of the file", "Own-learning share ÷ own share of file · 1.0 = no bias", ylabel="Model", lw=230, w=620))
        H.append(OV["self"])
    ...
    for m in ml:
        cr_ = written.get(m, 0) / W_all; us_ = ma[m] / U_all; tot_ = cr_ + us_
    ...
    OV["twoway"] = flex(stacked_h("Taste Created vs Used by Model", tw_pairs, list(tw_cols), tw_cols, "Balance of creating vs using taste", "Model", "Share of all taste created vs share of all taste used, normalized per model", w=620, unit="%", total=False), legend(tw_cols, "Direction"))
    H.append(two(OV.get("twoway", ""),
                 hbars("Taste Traffic per 100 Turns by Model", [(m, round(100 * (ma[m] + written.get(m, 0)) / max(1, pm[m]["asst"]), 1), "") for m in ml if pm[m]["asst"] >= 10], "Created + used, per 100 turns", "Learnings created + consulted, per 100 turns", ylabel="Model", lw=230, w=620)))
    ...
    if cot_ratio: H.append(two(hbars("Reasoning Length Ratio: Taste Turns vs Other Turns", cot_ratio, "Ratio (1.0 = no difference)", "Mean reasoning length on taste-consulting turns ÷ other thinking turns", ylabel="Model", lw=230, w=620),
                              hbars("Reasoning Share of Output by Model", [(m, r[4], "") for m, r in zip([r[0] for r in cot_rows], cot_rows)], "Percent of output tokens that are reasoning (est.)", "Estimated reasoning tokens ÷ output tokens", ylabel="Model", lw=230, w=620)))
    H.append(two(hbars("Taste Activations per 100 Turns by Model", [(m, round(100 * ma[m] / max(1, pm[m]["asst"]), 1), f"{ma[m]} activations over {pm[m]['asst']} turns") for m in ml], "Activations per 100 assistant turns", "Consultations per 100 assistant turns", ylabel="Model", lw=230, w=620),
                 hbars("Reasoning Volume per Turn by Model", [(m, round(pm[m]["think"] / max(1, pm[m]["asst"])), "") for m in ml], "Thinking characters per assistant turn", "Mean visible reasoning characters per turn", ylabel="Model", lw=230, w=620)))
    ...
    sub_t = f"One bubble per session · y = assistant turns · size = minutes · color = model · local time"
    ...
    for s in sorted(sessions, key=lambda s: -s["minutes"]):
        ...
        out.append(f"<circle cx='{tx(s['first']):.0f}' cy='{ty(s['asst']):.0f}' r='{r:.1f}' fill='{mcols.get(s['model'], 'var(--muted)')}' opacity='0.75' stroke='var(--bg)' stroke-width='1' data-model='{html.escape(s['model'], quote=True)}'><title>{esc(s['date'])} · {esc(s['title'] or s['sid'])}\n{esc(s['model'])}\n{s['user']} prompts · {s['asst']} turns · {s['minutes']} min · ${s['cost']:.2f} · {s['cites']} taste activations</title></circle>")
    leg_html = f"<div class=legend data-filter=timeline><b>Model</b>" + "".join(f"<div class=leg-item data-model='{html.escape(k, quote=True)}'><span class=dot style='background:{c}'></span> {esc(k)}</div>" for k, c in mcols.items()) + "</div>"
    OV["timeline"] = flex("".join(out) + "</svg>", leg_html)
    ...
    tools_chart = hbars("Tool Calls by Type", [(t, k, "") for t, k in tools.most_common(15)], "Calls", f"Edits per read {ed/max(1,rd):.2f} · shell calls per edit {tools.get('shell_command',0)/max(1,ed):.1f} · subagent spawns {tools.get('agent',0)}", ylabel="Tool", w=620, lw=170)
    ...
    hour_chart = flex(stacked_h("Prompts by Hour of Day (UTC)", [(f"{h:02d}:00", dict(hours[h])) for h in range(24) if hours.get(h)], ml, mcols, "Prompts", "Hour", "By session model", w=620), mleg)
    wday_chart = flex(stacked_h("Prompts by Weekday", [(w, dict(wdays[w])) for w in ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"] if wdays.get(w)], ml, mcols, "Prompts", "Day", "By session model", w=620), mleg)
    top_open = sorted(opens, key=lambda o: -sum(opens[o].values()))[:12]
    open_chart = flex(stacked_h("Prompt Openings by Model", [(o, dict(opens[o])) for o in top_open], ml, mcols, "Prompts", "Opening", f"By session model · {slash} slash commands excluded", w=620), mleg)
    len_keys = sorted(plens, key=lambda k: 10**6 if k == "1000+" else int(k.split("–")[0]))
    len_chart = flex(stacked_h("Prompt Length by Model", [(k, dict(plens[k])) for k in len_keys], ml, mcols, "Prompts", "Characters", "By session model", w=620), mleg)
    ph = by_hour([dict(_s=s_["model"], ts=p[0]) for s_ in sessions for p in s_["prompts"] if p[1].strip()], lambda a: a["ts"][:19])
    act_chart = tseries(f"Prompts {PER.title()}", [(l, dict(c)) for l, c in ph.items()], ml, mcols, BUCKET_WORD.capitalize(), "Prompts", w=620, h=300, leg=legend(mcols, "Model"))
    ...
    H.append(kpi([("bytes", fmt(len(raw)), ""), ("over 300 chars", len(long_b), "Long learnings read like incident reports rather than preferences and cost tokens every turn."), ("near-duplicates", len(dups), "Pairs of learnings sharing most of their words."), ("never activated", len(never), "Learnings never referenced in any recorded reasoning."), ("learn events in cmd", len(learn_events), "Times the agent called the taste tool inside a cmd session. Most learnings came from mining other agents' sessions instead.")]))
    size_chart = hbars("Base Prompt Size by Session", [(f"{s['date']} {s['title'] or s['sid']}", s["first_in"] or 0, "") for s in sessions], "First-turn input tokens", "First-turn input tokens · growth tracks the taste file", ylabel="Session", lw=330, w=1240, sort=False)
    len_bins = [(f"{lo}–{lo+99}", sum(1 for r in rows if lo <= r["n"] < lo + 100), "") for lo in range(0, 700, 100)] + [("700+", sum(1 for r in rows if r["n"] >= 700), "")]
    blen_chart = hbars("Learning Length Distribution", len_bins, "Number of learnings", "Characters per learning", ylabel="Characters", w=620, lw=110, sort=False)
    byday = collections.Counter(r["date"] for r in rows); W2, H2 = 1240, 190; GX0, GX1 = 70, W2 - 30
    out = [svg_open(W2, H2), title_block(W2, "Learnings Added over Time")]
    acc = 0; pts = []
    for dd in sorted(byday):
        ...
        out.append(R(px - 2, H2 - 30 - byday[dd] * 4, 4, byday[dd] * 4, "var(--muted)", f"{dd}: {byday[dd]} learnings"))
    ...
    conf_chart = hbars("Learning Confidence Distribution", [(f"{c:.2f}", k, "") for c, k in sorted(cc_hist.items(), reverse=True)], "Number of learnings", ylabel="Confidence", w=620, lw=90, sort=False)
    H.append(two(conf_chart, blen_chart)); H.append(size_chart); H.append(growth_chart); OV["size"] = size_chart
    h3("Never-activated learnings"); H.append("<p class=muted>Present in every prompt, never referenced in any recorded thinking. Candidates to trim.</p><details><summary>Show " + str(len(never)) + "</summary><ul>" + "".join(f"<li><span class=muted>#{r['i']} · {r['conf']:.2f}</span> {esc(r['text'][:200])}</li>" for r in never) + "</ul></details>")
...
H.append("<script>(function(){const tt=document.getElementById('tt');let cur=null;document.addEventListener('mousemove',e=>{const r=e.target.closest('rect.hit');if(!r){if(cur){cur.closest('g.bar')?.classList.remove('hot');cur.closest('svg')?.classList.remove('attn');cur=null;tt.style.opacity=0;}return;}if(r!==cur){if(cur){cur.closest('g.bar')?.classList.remove('hot');}cur=r;const g=r.closest('g.bar'),sv=r.closest('svg');if(g)g.classList.add('hot');if(sv)sv.classList.add('attn');tt.innerHTML=r.dataset.tip;tt.style.opacity=1;}const x=e.clientX+14,y=e.clientY+14;const bw=tt.offsetWidth,bh=tt.offsetHeight;const lx=Math.max(8,x+bw>innerWidth-8?e.clientX-bw-14:x);const ty=Math.max(8,y+bh>innerHeight-8?e.clientY-bh-14:y);tt.style.left=lx+'px';tt.style.top=ty+'px';});})();</script>")
...
H.append("<script>(function(){const containers=document.querySelectorAll('.legend[data-filter=timeline]');containers.forEach(container=>{const flex=container.closest('.flex');if(!flex)return;const circles=flex.querySelectorAll('circle[data-model]');const items=container.querySelectorAll('.leg-item');items.forEach(item=>{item.addEventListener('click',function(){const m=this.dataset.model;let ns=this.classList.contains('solo')?'inverted':this.classList.contains('inverted')?null:'solo';if(ns){items.forEach(it=>{it.classList.remove('solo','inverted');if(it.dataset.model===m){if(ns==='solo')it.classList.add('solo');else it.classList.add('inverted')}});circles.forEach(c=>{const cm=c.dataset.model;if(ns==='solo'){c.style.opacity=cm===m?'':0}else if(ns==='inverted'){c.style.opacity=cm===m?0:''}else c.style.opacity=''});}}});});})();</script>")
open(OUT, "w").write("\n".join(H))
print("wrote", OUT, "| learnings", len(rows), "| sessions", len(sessions), "| activations", len(acts), "steering", sum(1 for a in acts if a["steer"]))
if not A.no_open:
