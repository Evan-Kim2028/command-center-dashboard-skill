...
- **Cost**: `usage.costUsd` as logged; free tiers show 0. The Models tab regresses cost on (uncached, cached, output) tokens per model and reports the implied cached-vs-uncached price ratio, because some providers bill cached input at full rate.
